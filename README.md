...
**Деплой:** развёрнут одним сервисом на Render — в проде NestJS отдаёт и GraphQL API,
и собранный React-клиент (SPA-фолбэк для клиентских маршрутов). Один URL, без CORS.
...
- **Отзывчивый интерфейс** — адаптив под мобильные с бургер-меню, скелетоны загрузки,
  состояния ошибок и пустых списков, `ErrorBoundary`, доступность (aria, семантика).
...
# клиент
cd client
npm test            # 23 юнит-теста (Vitest + RTL): сторы и утилиты
npm run build       # tsc + vite build
```

CI на каждый push/PR (GitHub Actions) прогоняет typecheck, тесты и сборку обоих пакетов.

## 🧭 Возможные улучшения

Реальная БД (Postgres + Prisma) вместо JSON · авторизация и личный кабинет ·
URL-синхронизация фильтров каталога · кредитный калькулятор и запись на тест-драйв ·
сравнение авто · тёмная тема.
